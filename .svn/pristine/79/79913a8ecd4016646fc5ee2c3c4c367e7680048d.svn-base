...
public class ApplyMenus implements CycleLogic{
		...
		try {
			...
			if (StringHelper.isNotEmpty(authidArr)) {
				for (String authid : authidArr) {
					...
					/*String parentid = (String) mysession.selectOne("gsys.getMenusParentid", authid);
					if (parentid != null && !"0".equals(parentid)) {
						model = new RoleAuth();
						model.setUuid(Utils.createUUID());
						model.setRoleid(roleid);
						model.setType(type);
						model.setAuthid(parentid);

						mysession.delete("gsys.deleteRoleAuthBySelf", model);
						mysession.insert("gsys.insertRoleAuth", model);
					}*/
				}
			}
